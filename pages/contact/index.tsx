    ...
    <Layout pageTitle="contact">
      <div className={styles.hintText}>
        <p>
          ご連絡は以下のフォームまたは
          <a
            href="https://twitter.com/mogamin3"
            target="_blank"
            rel="noreferrer"
          >
            Twitter
      ...
      <div>
        ...
        >
          ...
          <div className={styles['contactForm__fieldContainer']}>
            <label className={styles['contactForm__label']} htmlFor="content">
              ✒️ 本文(400字まで)
            </label>
            <textarea
              id="content"
...
const useContactFormState = () => {
  type Action =
    | { type: 'inputName' | 'inputEmail' | 'inputContent'; input: string }
    | { type: 'reset' | 'startSend' | 'doneSend' };
  ...
  const isValidContent =
    state.content.trim().length > 0 && state.content.trim().length <= 400;

  const isValidForm = isValidName && isValidEmail && isValidContent;
  const onChangeName = (changeEvent: React.ChangeEvent<HTMLInputElement>) =>
    dispatch({ type: 'inputName', input: changeEvent.target.value });
  const onChangeEmail = (changeEvent: React.ChangeEvent<HTMLInputElement>) =>
    dispatch({ type: 'inputEmail', input: changeEvent.target.value });
  const onChangeContent = (
    changeEvent: React.ChangeEvent<HTMLTextAreaElement>
  ) => dispatch({ type: 'inputContent', input: changeEvent.target.value });
